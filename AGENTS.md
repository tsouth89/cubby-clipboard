...
- **`clips.content_hash` is indexed but not UNIQUE** — dedup relies entirely on the `SELECT uuid FROM clips WHERE content_hash = ?` lookup in `process_clipboard_snapshot` succeeding. The database will happily accept a second row for the same clip, so any code path that treats a failed lookup as "no such row" silently duplicates history instead of erroring. Handle the `Err` arm explicitly rather than collapsing it with `unwrap_or(None)`.
