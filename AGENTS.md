...
- **New `src/bin/*.rs` targets ship to users** — anything cargo emits into `target/<triple>/release/` gets bundled next to `cubby.exe`. Declare every diagnostic binary in `Cargo.toml` with `required-features = ["dev-harness"]`, or it lands unsigned in users' `%LOCALAPPDATA%`. `v1.2.6` shipped `clipboard_probe.exe` (725 KB) this way. CI fails the build if any executable other than `cubby.exe` appears in the release directory.
