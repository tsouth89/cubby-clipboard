...
- **The remote client's own keyboard forwarding must be off.** When a remote
  support client's "forward keyboard shortcuts" setting is on, the client installs
  its own keyboard capture that runs ahead of Cubby's hook and consumes the keys
  first, so the hotkey never reaches Cubby. With forwarding off, Cubby's hook wins
  and the hotkey works. Avoid `Win+V` itself inside a remote session with
  forwarding on: the Windows shell handles the Win key low enough that it can both
  open Cubby locally and leak to the remote.
...
Some remote support clients filter injected keyboard input, so Cubby cannot press
paste for you inside such a session. The reliable, driver-free flow is to let the
client synchronize the clipboard as data and paste it with a physical `Ctrl+V`:

Workflow:

1. Focus the destination inside the remote session.
...
large clipboard items. The client synchronizes the clipboard as data, so content
is pasted at once, not typed character by character.

The optional **Paste as keystrokes** mode invokes the remote client's own toolbar
action through Windows accessibility. It is useful for short text or sensitive
values, but the client deliberately types the content into the remote session and
is therefore unsuitable for large items.

Cubby does not synthesize Ctrl+V for these clients. Their players accept physical
keyboard input for forwarding but ignore the synthetic input methods available
to ordinary Windows applications.

## Other remote clients

Cubby also recognizes Windows Remote Desktop, AnyDesk, TeamViewer, ScreenConnect,
...
- `scripts/test-remote-trigger.ps1` validates the helper's direct activation
  channel and the configured-hotkey trigger (Ctrl+backtick) without requiring a
