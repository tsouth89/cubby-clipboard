...
This works only when the remote client's own keyboard forwarding is off. Some
remote support clients install their own keyboard capture when forwarding is on,
...
The trigger is active only for supported remote clients — Windows Remote Desktop,
AnyDesk, TeamViewer, ScreenConnect, Splashtop, RustDesk, and other recognized
remote support software — and only while replacement mode is enabled (that is
what runs the hook). It does nothing in ordinary local applications, where the
normal global shortcut already handles activation.
