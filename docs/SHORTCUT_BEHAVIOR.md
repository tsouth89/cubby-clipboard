...
required. The helper
intercepts only an exact physical `Win+V` chord and forwards it to Cubby's
registered `Win+Alt+V` shortcut. It ignores its own injected events, restores the
physical Windows-key state before unrelated input continues, and exits
independently if Cubby disables replacement mode.
